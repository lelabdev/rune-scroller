...
describe("DOM Utilities", () => {
  ...
  describe("createSentinel", () => {
    ...
    it("uses custom sentinelColor in debug mode", () => {
      const { element: sentinel } = createSentinel(
        testElement,
        true,
        0,
        "#ff0000",
    ...
    it("generates auto-ID when not provided", () => {
      ...
      expect(sentinel1.getAttribute("data-sentinel-id")).toMatch(
        /^sentinel-\d+$/,
      );
      expect(sentinel2.getAttribute("data-sentinel-id")).toMatch(
        /^sentinel-\d+$/,
      );
      // IDs should be different
      expect(sentinel1.getAttribute("data-sentinel-id")).not.toBe(
        sentinel2.getAttribute("data-sentinel-id"),
      );
    });
    ...
    it("always sets data-sentinel-id attribute", () => {
      const { element: sentinel1 } = createSentinel(testElement);
      const { element: sentinel2 } = createSentinel(
        testElement,
        false,
        0,
        "#00e0ff",
